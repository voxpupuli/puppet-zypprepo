...
#
#   EPOCH: An unsigned integer. This part is optional for backwards compatibility.
...
# zypper-versionlock.
#
# @example A complete, well-formed string: `bash-4.1.2-9.sles12_2.x86_64'
# @example A well-formed string that has dropped the optional BuildArch sub-field: `bash-4.1.2-9.suse15`
# @example A well-formed string using wildcards with epoch: `0:bash*-4.*-*.*`
...
#
# lint:ignore:140chars
type Zypprepo::VersionlockString = Pattern[/^(?:([0-9\*]+):)?([0-9a-zA-Z\._\+%\{\}\*-]+)-([^-]+)-([^-]+)\.(([0-9a-zZ-Z_\*]+)(?:\.(noarch|x86_64|i386|arm|ppc64|ppc64le|sparc64|ia64|alpha|ip|m68k|mips|mipsel|mk68k|mint|ppc|rs6000|s390|s390x|sh|sparc|xtensa|\*))?)$/]
# lint:endignore
